...
i2c = I2C(0, scl=Pin(17), sda=Pin(16))                 # Angir verdi og hvilken PIN 'er som skal brukes 
bme = BME280(i2c=i2c) 
ens = PiicoDev_ENS160(bus=0, scl=Pin(17), sda=Pin(16)) # Ditto
...
async def read_bme(): # Definerer funskjon som vil lese BME-sensor data
...
async def read_ens(): # Definerer funskjon som vil lese ENS-sensor data
    return ens.tvoc, ens.eco2.value, ens.eco2.rating
    
    
async def read_aht(): # Definerer funskjon som vil lese AHT-sensor data
    if aht_sensor.is_ready:
        temperature = aht_sensor.temperature
        humidity = aht_sensor.humidity  
    return temperature, humidity

def _pop0(l): # Begrenser antall avlesninger til 60 
    if len(l) >= 60:
        l.pop(0)

def _mid(l): # Midler verdien av de siste 60 verdiene
    return sorted(l)[len(l)//2]
    

async def update_sensors_data(data): # Definerer en funskjon som vil oppdatere sensor-data 
...
async def collect_sensors_data(data, test=False): # Definerer en funskjon som vil samle den oppdaterte sensorinformasjon    
    while True:
        await update_sensors_data(data)
        if test:
            print(data)
    
        await uasyncio.sleep_ms(5000) # Venter 5 sekunder førneste avlesninger
